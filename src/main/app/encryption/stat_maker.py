...
def make_stat():
    ...
    for filename in no_encr_files:
        cut_out, entropy, entropy_above_border, is_encr = calc_entropy(ed, ef, filename)
        write_result(cut_out, entropy, entropy_above_border, file_stat, filename, is_encr)
        if is_encr:
            fp += 1
        else:
            tn += 1
...
def calc_entropy(ed, ef, filename):
    ...
    return cut_out, entropy, entropy_above_border, is_encr


def write_result(cut_out, entropy, entropy_above_border, file_stat, filename, is_encr):
